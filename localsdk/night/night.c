...
// MQTT send info
bool night_state_mqtt(bool night, bool gray) {
    bool result = false;
    ...
    if(topic && topic[0]) {
        logger("night", "night_state_mqtt", LOGGER_LEVEL_INFO, "%s success.", "mqtt_fulltopic()");
    ...
    } else logger("night", "night_state_mqtt", LOGGER_LEVEL_ERROR, "%s error!", "mqtt_fulltopic()");
...
int night_mode_change_callback(int state) {
...
bool night_free() {
    ...
    // Disable night mode
    if(local_sdk_open_ircut() == LOCALSDK_OK) {
        logger("night", "night_free", LOGGER_LEVEL_INFO, "%s success.", "local_sdk_open_ircut()");
        if(local_sdk_close_night_light() == LOCALSDK_OK) {
    ...
    } else logger("night", "night_free", LOGGER_LEVEL_ERROR, "%s error!", "local_sdk_open_ircut()");
