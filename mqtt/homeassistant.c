...
static char *mqtt_homeassistant_json_default_entity_id = "";
...
static bool mqtt_homeassistant_json_sensor(yyjson_mut_doc *json_doc, yyjson_mut_val *json_root, char *topic_name, char *json_field, char *device_class, char *unit_of_measurement, bool enabled) {
    ...
    if(result &= (asprintf(&mqtt_homeassistant_json_object_id, "%s_%s_%s_%s", mqtt_topic, general_name, topic_name, json_field) != -1)) {
        ...
        if(result &= yyjson_mut_obj_add_str(json_doc, json_root, "object_id", mqtt_homeassistant_json_object_id)) {
            ...
            // Default entity id
            if(result &= (asprintf(&mqtt_homeassistant_json_default_entity_id, "sensor.%s_%s_%s_%s", mqtt_topic, general_name, topic_name, json_field) != -1)) {
                LOGGER(LOGGER_LEVEL_DEBUG, "%s success.", "asprintf(mqtt_homeassistant_json_default_entity_id)");
                if(result &= yyjson_mut_obj_add_str(json_doc, json_root, "default_entity_id", mqtt_homeassistant_json_default_entity_id)) {
                    LOGGER(LOGGER_LEVEL_DEBUG, "%s success.", "yyjson_mut_obj_add_str(default_entity_id)");
                    // Unique id
                    ...
                    free(client_id);
                } else LOGGER(LOGGER_LEVEL_WARNING, "%s error!", "yyjson_mut_obj_add_str(default_entity_id)");
            } else LOGGER(LOGGER_LEVEL_WARNING, "%s error!", "asprintf(mqtt_homeassistant_json_default_entity_id)");
...
static bool mqtt_homeassistant_json_binary_sensor(yyjson_mut_doc *json_doc, yyjson_mut_val *json_root, char *topic_name, char *json_field, char *device_class, int off_delay, bool enabled) {
    LOGGER(LOGGER_LEVEL_DEBUG, "Function is called...");
    bool result = true;
    ...
    if(result &= (asprintf(&mqtt_homeassistant_json_object_id, "%s_%s_%s_%s", mqtt_topic, general_name, topic_name, json_field) != -1)) {
        ...
        if(result &= yyjson_mut_obj_add_str(json_doc, json_root, "object_id", mqtt_homeassistant_json_object_id)) {
            LOGGER(LOGGER_LEVEL_DEBUG, "%s success.", "yyjson_mut_obj_add_str(object_id)");
            // Default entity id
            if(result &= (asprintf(&mqtt_homeassistant_json_default_entity_id, "binary_sensor.%s_%s_%s_%s", mqtt_topic, general_name, topic_name, json_field) != -1)) {
                LOGGER(LOGGER_LEVEL_DEBUG, "%s success.", "asprintf(mqtt_homeassistant_json_default_entity_id)");
                if(result &= yyjson_mut_obj_add_str(json_doc, json_root, "default_entity_id", mqtt_homeassistant_json_default_entity_id)) {
                    LOGGER(LOGGER_LEVEL_DEBUG, "%s success.", "yyjson_mut_obj_add_str(default_entity_id)");
                    // Unique id
                    ...
                    if(result &= (asprintf(&mqtt_homeassistant_json_unique_id, "%s_%s_%s", client_id, topic_name, json_field) != -1)) {
                        ...
                        if(result &= yyjson_mut_obj_add_str(json_doc, json_root, "unique_id", mqtt_homeassistant_json_unique_id)) {
                            ...
                            // Off delay
                            if(off_delay > 0) {
                                if(result &= yyjson_mut_obj_add_int(json_doc, json_root, "off_delay", off_delay)) {
                                    LOGGER(LOGGER_LEVEL_DEBUG, "%s success.", "yyjson_mut_obj_add_int(off_delay)");
                                } else LOGGER(LOGGER_LEVEL_WARNING, "%s error!", "yyjson_mut_obj_add_int(off_delay)");
                            }
                            // Enable by default
                            if(result &= yyjson_mut_obj_add_bool(json_doc, json_root, "enabled_by_default", enabled)) {
                                LOGGER(LOGGER_LEVEL_DEBUG, "%s success.", "yyjson_mut_obj_add_bool(enabled_by_default)");
...
bool mqtt_homeassistant_discovery(int type, char *topic_name, char *json_field, char *device_class, char *unit_of_measurement, int off_delay, bool enabled) {
    LOGGER(LOGGER_LEVEL_DEBUG, "Function is called...");
    bool result = true;
        ...
        if(result &= mqtt_homeassistant_json_device(json_doc, json_root)) {
            ...
            switch(type) {
                ...
                case MQTT_HOMEASSISTANT_BINARY_SENSOR:
                    ...
                    if(result &= mqtt_homeassistant_json_binary_sensor(json_doc, json_root, topic_name, json_field, device_class, off_delay, enabled)) {
            ...
            if(result) {
                ...
                free(mqtt_homeassistant_json_default_entity_id);
